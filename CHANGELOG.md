...
### StatusCode 副本豁免注解恢复 + 头注释副本视角修正（第十三轮深度审计）

- 类头恢复 `@audit-ignore fqcn_dup` 豁免注解（v1.0.10 同步时丢失 → rocareer:audit
  fqcn_dup 门禁误报 FQCN 重复；Install.php 头注释本就声明该注解约定，本版对齐）
- 头注释修正为副本视角：原同步把 radmin 真源的「本文件为唯一真源」说明一并拷入，
  副本自称真源语义错位；现明确「同步副本 + 落盘用途 + 同步须保留豁免注解」
- 内容与 radmin v5.2.17 真源逐字节对齐（仅头注释差异）
